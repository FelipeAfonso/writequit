<script lang="ts">
	...
	import type { TagType } from '@writequit/ui';
	...
	const TAG_TYPES: { value: TagType | undefined; label: string }[] = [
	...
	let sorted = $derived(allTags.current ? sortTags(allTags.current) : []);
	...
	async function handleTypeChange(tagId: string, type: TagType | undefined) {
		try {
			await provider.tags.update(tagId, { type });
		} catch (error) {
	...
	async function confirmDelete() {
		...
		try {
			await provider.tags.remove(deleteTargetId);
		} catch (error) {
	...
	function typeIndex(type: TagType | undefined): number {
	...
	function cycleTypeForward() {
		...
		handleTypeChange(tag.id, TAG_TYPES[next].value);
	}
...
<div class="mx-auto flex max-w-3xl flex-col gap-6 p-6">
	...
	<div class="flex items-baseline gap-3">
		...
		{#if allTags.current}
			<span class="font-mono text-xs text-fg-muted">
				{allTags.current.length} tag{allTags.current.length !== 1 ? 's' : ''}
			</span>
		{/if}
	</div>
	...
	{:else if allTags.current && allTags.current.length === 0}
		<div class="py-8 text-center font-mono text-sm text-fg-muted">
			No tags yet. Create a task with +tagname to get started.
		</div>
	{:else if allTags.current}
					...
					<div class="flex items-center gap-1.5">
						...
						{#each TAG_TYPES as opt (opt.label)}
							<button
								...
								onclick={() => handleTypeChange(tag.id, opt.value)}
							>
								{opt.label}
							</button>
						{/each}
